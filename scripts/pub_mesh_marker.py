...
def table_marker_publisher():
    ...
    if not rospy.is_shutdown():
        msg = MarkerArray()
        q = Quaternion(x=-0.0928, y=0.7012, z=0.7008, w=-0.0916)
        msg.markers.append(make_mesh_marker("l_gripper_tool_frame", len(msg.markers) + 1, Pose(Point(x=0.14, y=0.03), q), Vector3(1, 1, 1), white(), "package://giskard_ros/object_meshes/spatula.dae", frame_locked=True))
        pub.publish(msg)
